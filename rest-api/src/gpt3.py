...
INITIAL_PROMPT_PLACEHOLDER = (
    "Generate up to 3 categories from a given text. Categories should be comma-separated."
...
def split_tags_from_date(raw_tags: Tags) -> Tuple[Tags, List[datetime]]:
    dates = [
        search_dates(tag, languages=["en"]) for tag in raw_tags
    ]  # TODO: change language by user's preference
    ...
    for date_i, date in enumerate(dates):
        if date is not None:
            ...
            parsed_dates.append(
                date[0][1]
            )  # TODO: currently getting 1st date only - get all and try to join them
    tags_wo_dates = [
        tag for tag_i, tag in enumerate(raw_tags) if tag_i not in date_tag_ix
    ]
    return tags_wo_dates, parsed_dates


def finalize_tags_with_gpt3(
    raw_tags: Tags, prompt_placeholders: List[str], models: List[str]
) -> Tags:
    tags = raw_tags
...
def get_tags_and_date(
    sentence: str, model: str = "text-davinci-003"
) -> Record:
    with open(PATH_TO_KEY_FILE, "r") as f:
        openai.api_key = f.read()
    response = get_initial_response(
        sentence, INITIAL_PROMPT_PLACEHOLDER, model=model
    )
    raw_tags = response2tags(response)
    tags, dates = split_tags_from_date(raw_tags)
    date = (
        dates[0] if dates else None
    )  # TODO: remove hardcoded 1st date selection
    tags = finalize_tags_with_gpt3(
        raw_tags, PROCESSING_TAGS_PLACEHOLDERS, PROCESSING_MODELS
    )
